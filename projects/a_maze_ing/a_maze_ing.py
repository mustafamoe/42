...
def interactive_loop(
    ...
    print("\nCommands: new, path, color NAME, save, help, quit")
    while True:
        try:
            command = input("maze> ").strip()
        except EOFError:
            print()
            return
        if command in ("quit", "q", "exit"):
            return
        if command == "help":
            print("Commands: new, path, color NAME, save, help, quit")
            continue
        if command == "path":
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print_result(generator, out_path, show_path, color)
        if interactive:
            interactive_loop(config, config_path, generator, show_path, color)
        return 0
